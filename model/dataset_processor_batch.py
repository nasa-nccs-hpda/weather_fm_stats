...
class BatchDatasetProcessor:
    ...
    def _regrid_variables_batch(self, regridder, ds_levels, pending_vars,
                                std_coords):
        '''Regrid a batch of variables in one xESMF call when possible.'''
        if not pending_vars:
            return {}

        batch_ds = xr.Dataset()
        for output_name, source_name in pending_vars.items():
            try:
                batch_ds[output_name] = self._make_regrid_input_contiguous(
                    ds_levels[source_name])
            except Exception as exc:
                raise ValueError(
                    f'Failed to prepare regridding input for {output_name} '
                    f'({source_name}): {exc}')

        try:
            regridded_ds = regridder(batch_ds)
        except Exception as exc:
            raise ValueError(f'Batch regridding failed: {exc}')

        regridded_vars = {}
        for output_name in pending_vars:
            regridded_data = regridded_ds[output_name]
            if 'time' in regridded_data.coords:
                regridded_data = regridded_data.assign_coords(time=std_coords)
            regridded_vars[output_name] = regridded_data
    ...
    def _process_dataset_variables(self, ds, colls_for_file, file_type,
        ...
        # Group variables into one batch regrid call to reduce per-variable
        # xESMF overhead and avoid repeated lazy-array preparation.
        regridded_vars = {}
        pending_vars = {}
        ...
        for target_var, source_var in found_vars.items():
            if source_var == 'calculated':
                ...
                if target_var in calculated_vars:
                    for dep_var, dep_info in calculated_vars[target_var].items():
                        ...
                        if (dep_info['collection'] in colls_for_file
                            ...
                            and dep_var not in pending_vars):
                            dep_source = dep_info['alias']
                            pending_vars[dep_var] = dep_source
                            print(f'  [INFO] Queued dependency {dep_var} '
                                  f'(alias: {dep_source}) for calculated '
                                  f'variable {target_var}')
                continue  # Skip the calculated variable itself

            if target_var not in regridded_vars and target_var not in pending_vars:
                pending_vars[target_var] = source_var

        if pending_vars:
            try:
                batch_regridded = self._regrid_variables_batch(
                    regridder, ds_levels, pending_vars, std_coords)
                regridded_vars.update(batch_regridded)
            except Exception as exc:
                print(f'[ERROR] Batch regridding failed: {exc}')
                raise
    ...
    def save_processed_datasets(self, results, info_dir=None,
        ...
        if single_fcst_mode:
            ...
            print('\n[INFO] Single-forecast mode: Keeping datasets in memory '
                  '(not saving to disk)')
            return
        ...
        for ds_nm, ds in datasets.items():
            ...
            if chunk_id is not None:
                ...
                filenm = os.path.join(
                    f'outputs/{info_dir}/tmp',
                    self._generate_output_filenm(
                        ds_nm, target_coll=target_coll, info_dir=info_dir))
            else:
                filenm = os.path.join(
                    'outputs', self._generate_output_filenm(ds_nm))

            if ds_nm == 'fcst':
                cds_to_keep = ['init_date', 'lead', 'lev', 'lat', 'lon']
            elif ds_nm in ['ana', 'clim']:
                cds_to_keep = ['time', 'lev', 'lat', 'lon']
            else:
                cds_to_keep = list(ds.coords)
            ...
            for var in ds.data_vars:
                ...
                for dim in dims_to_drop:
                    if dim in var_data.dims:
                        var_data = var_data.isel({dim: 0}, drop=True)
                ...
                if not skip_calc_mode:
                    ...
                    if var_upper in VARS_LONG_MAP:
                        var_data.attrs['long_name'] = VARS_LONG_MAP[var_upper]
                    if var_upper in VARS_UNIT_MAP:
                        var_data.attrs['units'] = VARS_UNIT_MAP[var_upper]
                data_vars[var] = var_data

            clean_coords = {coord: ds.coords[coord] for coord in cds_to_keep
            ...
            if ds_nm == 'fcst':
                self._add_exclusion_metadata(ds_clean, self.config)

            if date_start_idx is not None and ds_nm == 'fcst':
                ds_clean.to_netcdf(
                    filenm, unlimited_dims=['init_date'], encoding=encoding)
            else:
        ...
        if existing_datasets:
            print(f'[INFO] Used {len(existing_datasets)} existing datasets')
