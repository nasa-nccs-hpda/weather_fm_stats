...
def load_stats_data(yaml_file: str, exp: int, process: str, part: int
    ...
    Returns a dictionary containing statistics data and configutation:
        ...
        - 'date_nms': List of initialization dates as strings ('%Y%m%d_%H')
    ...
    def merge_experiment_files(file_list, exp_name):
        ...
        for f in file_list:
            ...
            if excl_match:
                ...
                # Parse excluded dates (handle comma-separated values)
                if ',' in excluded_dates_str:
                    excluded_list = [d.strip() for d in 
                                     excluded_dates_str.split(',')]
                else:
        ...
        if len(datasets) > 1: # multiple files
            ...
            for i, ds in enumerate(datasets):
                ...
                if overlap:
                    overlap_strs = [d.astype('datetime64[us]').astype(
                        datetime).strftime('%Y%m%d_%H') for d in overlap]
                    # Clean up
                    for ds_cleanup in datasets:
                        ds_cleanup.close()
                    sys.exit(f'ERROR: Duplicate dates found in {exp_name} files:\n'
            ...
            ds_with_init = xr.concat([ds[vars_with_init_date] for ds in datasets], 
                                    dim='init_date').sortby('init_date')
            
            
            # Weighted averaging for _avg and _glo variables
            total_dates = sum(date_counts)
            ds_avg_weighted = datasets[0][vars_avg] * (date_counts[0]/total_dates)
            for i in range(1, len(datasets)):
                weight = date_counts[i] / total_dates
                ds_avg_weighted = ds_avg_weighted + datasets[i][vars_avg] * weight
            ds_glo_weighted = datasets[0][vars_glo] * (date_counts[0]/total_dates)
            for i in range(1, len(datasets)):
                weight = date_counts[i] / total_dates
                ds_glo_weighted = ds_glo_weighted + datasets[i][vars_glo] * weight
            
            # For non-_avg/glo variables without init_date, take from first dataset
            ds_non_avg = first_ds[vars_non_avg_without_init]
            # Combine everything back together
            merged_ds = xr.merge([ds_with_init, ds_avg_weighted, ds_glo_weighted, 
                                  ds_non_avg])
    ...
    for exp_idx, file_list in enumerate(exp_file_lists):
        ...
        for file_idx, exp_file in enumerate(file_list):
            ...
            file_suffix = '_'.join(suffix_parts[1:])
    ...
    for i, file_list in enumerate(exp_file_lists):
        ...
        for p in suffix_config_parts:
            if p.startswith('len') or p.startswith('int'):
                flex_parts.append(p)
            elif p.startswith('spc') or 'x' in p:
                inflex_parts.append(p)
        exp_inflex_configs.append('_'.join(flex_parts))
        exp_flex_configs.append('_'.join(inflex_parts))
    ...
    elif len(set(exp_flex_configs)) == 1:
    ...
    # Validate exclusion consistency across experiments (if exclusions exist)
    if any(len(excluded_set) > 0 for excluded_set 
           in experiment_excluded_dates):
        # Check that all experiments have identical exclusions
        first_excluded = experiment_excluded_dates[0]
        for exp_idx, excluded_set in enumerate(
                experiment_excluded_dates[1:], 1):
            if excluded_set != first_excluded:
                exp0_exs = sorted(first_excluded) if first_excluded else 'none'
                expN_exs = sorted(excluded_set) if excluded_set else 'none'
                sys.exit(f'ERROR: Experiments have different excluded dates:\n'
                         f'  exp0: {exp0_exs}\n  exp{exp_idx}: {expN_exs}\n'
                         f'All experiments must have identical exclusions.')
        
        # Store the consistent exclusion set
        excluded_dates = first_excluded
        total_files_with_exclusions = sum(experiment_exclusion_counts)
        
        print('\nExclusion Analysis:')
        print('  All experiments have identical exclusions: ✓')
        print(f'  Excluded dates: {", ".join(sorted(excluded_dates))} '
              f'(total: {len(excluded_dates)})')
        print(f'  Files with exclusions: {total_files_with_exclusions}/'
              f'{sum(len(file_list) for file_list in exp_file_lists)} total '
              f'files across all experiments')
    else:
        excluded_dates = set()
    ...
    if start_date and end_date and fcst_spacing:
        ...
        try:
            ...
            while current_dt <= end_dt:
                requested_dates.append(current_dt.strftime('%Y%m%d_%H'))  
        ...
        requested_dates = [d.astype('datetime64[us]').astype(
            datetime).strftime('%Y%m%d_%H') for d in unique_dates]
    ...
    # Apply exclusions to requested dates
    if excluded_dates:
        original_count = len(requested_dates)
        requested_dates = [date for date in requested_dates 
                           if date not in excluded_dates]
        excluded_count = original_count - len(requested_dates)
        print(f'  Date processing: {original_count} original → '
              f'{len(requested_dates)} requested ({excluded_count} excluded)')
    ...
    if process in ['indiv', 'comp']: 
        ...
        if excluded_dates:
            ...
            print(f'Requested Dates ({len(requested_dates)} after '
                  f'exclusions): [{dates_listing}]')
        else:
            dates_listing = [', '.join(requested_dates[:5] + (
                ['...'] if len(requested_dates) > 5 else []))]
        ...
        for exp_idx, file_list in enumerate(exp_file_lists):
            ...
            available_date_nms = [d.astype('datetime64[us]').astype(
                datetime).strftime('%Y%m%d_%H') for d in init_dates]
            ...
            if missing_dates:
                ...
                sys.exit(f'ERROR: exp{exp_idx} is missing required dates:\n'
                         f'  Missing: {missing_strs[:10]}'
                         f'{"..." if len(missing_strs) > 10 else ""}\n'
                         f'  Total missing: '
                         f'{len(missing_dates)}/{len(requested_dates)} dates\n'
                         f'  All requested dates must be present in each '
                         f'experiment.')
            ...
            if extra_dates:
                ...
                ds.close()
                sys.exit(f'ERROR: exp{exp_idx} contains extra dates not in '
                         f'requested range:\n  Extra: {extra_strs[:10]}'
                         f'{"..." if len(extra_strs) > 10 else ""}\n'
                         f'  Total extra: {len(extra_dates)} dates\n'
                         f'  Datasets cannot contain extra dates (because of '
                         f'averages).')
            print(f'Date validation: Perfect match - '
                  f'{len(available_date_nms)} dates')
            
            # Filter dataset to requested dates maintaining order
