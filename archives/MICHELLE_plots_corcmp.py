...
def load_stats_data(yaml_file: str, process: str, part: int
    ...
    experiment_valid_dates = []  # Valid dates per experiment after initial merging/filtering
    ...
    for i in range(len(exp_file_lists)):
        if exp_lens[i] < fcst_length:
            timing_issues.append(f'  exp{i}: Shorter forecast length ({exp_lens[i]}d < {fcst_length}d)')
        if exp_ints[i] > fcst_interval:
            timing_issues.append(f'  exp{i}: Coarser forecast interval ({exp_ints[i]}h > {fcst_interval}h)')
        if exp_spcs[i] > fcst_spacing:
            timing_issues.append(f'  exp{i}: Coarser forecast spacing ({exp_spcs[i]}d > {fcst_spacing}d)')
    
    if timing_issues:
        error_msg = 'ERROR: Incompatible timing parameters found:\n'
        error_msg += '\n'.join(timing_issues)
        error_msg += '\nExperiments must have equal or longer length, equal or finer interval, '
        error_msg += 'and equal or finer spacing than requested parameters.'
        sys.exit(error_msg)
    ...
    if all_match:
        ...
        for i in range(len(exp_file_lists)):
            if exp_lens[i] != fcst_length or exp_ints[i] != fcst_interval or exp_spcs[i] != fcst_spacing:
                print(f'  exp{i}: len{exp_lens[i]}d_int{exp_ints[i]}h_spc{exp_spcs[i]}d')
    ...
    if process == 'plot': 
        ...
        for exp_idx, file_list in enumerate(exp_file_lists):
            ...
            if missing_dates:
                # Check if all missing dates are in this experiment's exclusion set
                missing_and_not_excluded = missing_dates - excluded_dates_set
                if missing_and_not_excluded:
                    missing_strs = [str(d) for d in sorted(missing_and_not_excluded)]
                    ds.close()
                    sys.exit(f'ERROR: exp{exp_idx} is missing required dates that are not excluded:\n'
                             f'  Missing non-excluded dates: {missing_strs[:10]}'
                             f'{"..." if len(missing_strs) > 10 else ""}\n'
                             f'  Total missing and not excluded: '
                             f'{len(missing_and_not_excluded)}/{len(requested_dates)} dates')
                else:
                    print(f'  Note: {len(missing_dates)} requested dates are missing but in exclusion list')
            
            # Filter dataset to only include requested dates (remove extras)
            available_requested = sorted(list(set(available_date_nms) & set(requested_dates)))
            original_date_count = len(available_date_nms)
            date_indices = [available_date_nms.index(date) for date in available_requested]
            ds = ds.isel(init_date=date_indices)
            print(f'Date validation: {original_date_count} → {len(available_requested)} dates (after removing extras)')
        ...
        if all(set(exp_dates) == set(experiment_valid_dates[0]) for exp_dates in experiment_valid_dates):
        # No filtering of arrays: all experiments have same dates
            if all_excluded_dates:
                ...
                print(f'Found exclusions across experiments: {sorted(all_excluded_dates)}')
                original_count = len(requested_dates)
                requested_dates = [date for date in requested_dates if date not in all_excluded_dates]
                print(f'Applied exclusions: {original_count} → {len(requested_dates)} dates')
        else:
...
def calc_ci(ctl_data, model_data, coll, sig):
    '''
    Calculate assymetric confidence intervals for model-control differences
    Parameters:
